...
import { ChildCareDoctorsFetch } from '../Slices/AboutSlice'




export default function ChildcareDoctors() {

    const dispatch = useDispatch()

    const ChildCareDoctors = async () => {
        try {
            const response = await dispatch(ChildCareDoctorsFetch())
    ...
    const { isPending, isError, data, error } = useQuery({
        queryKey: ['childcaredoctors'],
        queryFn: ChildCareDoctors,
    })
    ...
    console.log("CHILD__CARE__DATAAAA>....", data[0]?.doctor_id 
    )
